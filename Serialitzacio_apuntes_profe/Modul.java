...
String nom="";
switch(k){
case 1: System.out.println("Quin es el nou nom del modul?");
 nom=sc.next();
this.canviaNom(nom);
break;
 case 2: System.out.println("Quin sera el nom del nou professor?");
 nom=sc.next();
this.canviaProfe(nom);
break;
 case 3: this.omplirUFs(sc);
break;
 case 4: System.out.println("Quina es la UF que vols rectificar?");
 int r=sc.nextInt();
System.out.println("Quin es el nou titol de la UF?");
nom=sc.next(nom);
this.canviaUf(r-1,nom);
break;
 case 5: System.out.println("Quina es la UF que vols treure?");
 int e=sc.nextInt();
this.ufs.remove(e-1);
break;
 }
