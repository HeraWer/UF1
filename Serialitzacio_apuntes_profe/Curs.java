...
m.llistarUfs();
 ...
 /* (1) L'iterador serveix per poder fer
 fer un recorregut del HashMap moduls. El
 metode entrySet retorna un Set, que es un
 conjunt d'elements no repetits, que
treballarem
 pero amb Map, una interface que olfereix
 metodes que ens donen cadascuna de les
parelles
 d'objectes i no cadascun d'ells de forma
 independent.
 */
...
String codi="";
switch(k){
 case 1: System.out.println("Quin es el nom del nou tutor?");
 codi=sc.next();
this.canviaTutor(codi);
break;
 case 2: this.omplirAlumnes(sc);
break;
 case 3: System.out.println("Quin numero te l'alumne?");
int b=sc.nextInt();
String
alumne=(String)this.alumnes.get(b-1);
System.out.println("Vols eliminar a "+alumne+" (s/n)?");
String sn2=sc.next();
if(sn2.equals("s"))
 this.treureAlumne(b-1);
break;
 case 4: System.out.println("Quina clau te el modul?");
codi=sc.next();
Modul m=new Modul();
m.omplirModul(sc);
...
break;
 case 5: System.out.println("Quina clau te el modul?");
codi=sc.next();
Modul mm=new Modul();
...
break;
 case 6: System.out.println("Quina clau te el modul?");
codi=sc.next();
this.moduls.remove(codi);
